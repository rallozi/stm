##########################################################################
#1. NR_jj<- est_j - learning.rate * solve(hess_jj) %*% grad_jj
#2. Convergence based on relative absolute change in likelihood
##########################################################################
`kalman` <-
function (z, dist, r, tp, n, p, phi_j, covariates, SigmaEta, learning.rate) {
...
all.R<-lapply(kalman.res, function(out) {
  out$filter$R
})

all.a<-lapply(kalman.res, function(out) {
  out$filter$a
})
...
##SETTING 2###
##############
dQdgamma_jj<-dQdgamma(m=m, tp=tp, sigma2omega=sigma2omega_jj, GammaInv=GammaInv, W=W)
d2Qd2gamma_jj<-d2Qd2gamma(m=m, tp=tp, sigma2omega=sigma2omega_jj, GammaInv=GammaInv, W=W)

dQdnu_jj<-dQdnu(m=m, tp=tp, GammaInv=GammaInv, sigma2omega=sigma2omega_jj, W=W, d=dist, s=phi_j$s, nu=phi_j$nu)
d2Qd2nu_jj<-d2Qd2nu(m=m, tp=tp, GammaInv=GammaInv, sigma2omega=sigma2omega_jj, W=W, d=dist, s=phi_j$s, nu=phi_j$nu)
...
d2Qdgammadnu_jj<-d2Qdgammadnu(m=m, tp=tp, GammaInv=GammaInv, sigma2omega=sigma2omega_jj, W=W, d=dist, s=phi_j$s, nu=phi_j$nu)
d2Qdgammads_jj<-d2Qdgammads(m=m, tp=tp, GammaInv=GammaInv, sigma2omega=sigma2omega_jj, W=W, d=dist, s=phi_j$s, nu=phi_j$nu)
d2Qdnuds_jj<-d2Qdnuds(m=m, tp=tp, GammaInv=GammaInv, sigma2omega=sigma2omega_jj, W=W, d=dist, s=phi_j$s, nu=phi_j$nu)

grad_jj<-matrix(c(dQdgamma_jj, dQdnu_jj, dQds_jj), nrow=3)

hess_jj<-matrix(c(d2Qd2gamma_jj,   d2Qdgammadnu_jj, d2Qdgammads_jj,
                  d2Qdgammadnu_jj, d2Qd2nu_jj,      d2Qdnuds_jj,
                  d2Qdgammads_jj,  d2Qdnuds_jj,     d2Qd2s_jj), 
                nrow=nrow(grad_jj), 
                ncol=nrow(grad_jj), 
                byrow=TRUE)

est_j<-matrix(c(phi_j$b, phi_j$nu, phi_j$s), nrow=3)
NR_jj<- est_j - learning.rate * solve(hess_jj) %*% grad_jj
...
return(list(phi=phi_jj, likelihood=loglik_jj, all.a=all.a, all.R=all.R))
}
